...
function createFireDataStructere() {
...
function updateFireItensityPerPixel (currentPixelIndex) {
    ...
    const belowPixelFireIndex = firePixelArray[belowPixelIndex]
    const newFireIntensity = belowPixelFireIndex - decay >= 0 ? belowPixelFireIndex - decay : 0
...
function renderFire () {
   ...
   for (let row = 0; row < fireHeight; row++) {
        ...
        for (let collumn = 0; collumn < fireWidth; collumn++) {
            const pixelIndex = collumn + (fireWidth * row)
            const fireItensity = firePixelArray[pixelIndex]
            if (debug === true) {
                html +='<td>'
                ...
                const color = fireColorPalette [fireItensity]
...
function createFireSource () {
    for (let collumn = 0; collumn <= fireWidth; collumn++) {
        ...
        const pixelIndex = (overFlowPixel-fireWidth) + collumn
